...
export default async function sendRequest( configuration: sendRequestProps ) {
   return new Promise<any>(async (resolve, reject) => {
      ...
      .catch((error) => {
         ...
         if (error.status == 401  && (!configuration.url.startsWith('authentication') || configuration.url.startsWith('authentication/status'))) { // I understand this is hard coded, I do plan on correcting this in the future
